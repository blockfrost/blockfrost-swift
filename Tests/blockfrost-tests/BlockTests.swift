...
final class BlockTests: QuickSpec {
    ...
    override func spec() {
        ...
        describe("blocks") {
            ...
            it("getLatestTxsAll"){
                waitUntil(timeout: TestConsts.TIMEOUT) { done in
                    let _ = api.getTransactionsInLatestBlockAll() { resp in
                        guard let r = TestUtils.getResult(resp: resp) else {
                            done(); return;
                        }
            ...
            if #available(macOS 12.0, iOS 15.0, tvOS 15.0, watchOS 8.0, *) {
                it("getLatestTxsAsync") {
                    self.runAsyncTest {
                        let r = try await api.getTransactionsInLatestBlockAsync()
                        expect(r).toNot(beNil())
                    }
                }

                it("getLatestTxsAllAsync") {
                    self.runAsyncTest {
                        let r = try await api.getTransactionsInLatestBlockAllAsync()
                        expect(r).toNot(beNil())
                    }
                }
            }
            ...
            it("getGenesis"){
                waitUntil(timeout: TestConsts.TIMEOUT) { done in
                    let _ = api.getBlock(hashOrNumber: "5f20df933584822601f9e3f8c024eb5eb252fe8cefb24d1317dc3d432e940ebb") { resp in
                        ...
                        expect(r).toNot(beNil())
                        expect(r.time).to(equal(1506203091))
                        expect(r.height).to(beNil())
                        expect(r.hash).to(equal("5f20df933584822601f9e3f8c024eb5eb252fe8cefb24d1317dc3d432e940ebb"))
                        expect(r.slot).to(beNil())
                        expect(r.epoch).to(beNil())
                        expect(r.epochSlot).to(beNil())
                        expect(r.slotLeader).to(equal("Genesis slot leader"))
                        expect(r.size).to(equal(0))
                        expect(r.txCount).to(equal(14505))
                        expect(r.output).to(equal("31112484745000000"))
                        expect(r.fees).to(equal("0"))
                        expect(r.blockVrf).to(beNil())
                        expect(r.previousBlock).to(beNil())
                        expect(r.nextBlock).to(equal("89d9b5a5b8ddc8d7e5a6795e9774d97faf1efea59b2caf7eaf9f8c5b32059df4"))
                        expect(r.confirmations).to(beAKindOf(Int.self))
                        done()
                    }
                }
            }
            ...
            it("previous"){
                waitUntil(timeout: TestConsts.TIMEOUT) { done in
                    let _ = api.getPreviousBlocks(hashOrNumber: "5360435", count: 1) { resp in
                        ...
                        expect(r[0].height).to(equal(5360434))
                        expect(r[0].time).to(equal(1613692790))
                        expect(r[0].slot).to(equal(22126499))
                        expect(r[0].epoch).to(equal(248))
                        expect(r[0].epochSlot).to(equal(353699))
                        expect(r[0].hash).to(equal("e8b8fd03492a0d2eeb19481e3a46080af271bf7f34a9d29a1fac44ee35414d7e"))
                        expect(r[0].size).to(equal(355))
                        expect(r[0].txCount).to(equal(1))
                        expect(r[0].fees).to(equal("172101"))
                        expect(r[0].blockVrf).to(equal("vrf_vk1s3hssxtpjdjc9amqn9wqn6dfyzujsv8jrk6pkl4esfpp99v48ujsflsn70"))
                        expect(r[0].previousBlock).to(equal("ca74257598fec490fac4303d0e243ee0b4e17c7a0f291597e4a35a89a7624d09"))
                        expect(r[0].nextBlock).to(equal("c6e3901b2cd95598b0bf5dc55cb62d682ea369298be9825b4b58d891b844b320"))
                        done()
                    }
                }
            }
            ...
            it("txsAll"){
                waitUntil(timeout: TestConsts.TIMEOUT) { done in
                    let _ = api.getBlockTransactionsAll(hashOrNumber: "5549919") { resp in
                        guard let r = TestUtils.getResult(resp: resp) else {
                            done(); return;
                        }
